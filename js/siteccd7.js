...
$(document).ready(function() {
	...
	$('.scrollCue, .scrollCueVideo, .intro .logo').click(function() {
	    $('html, body').animate({
				scrollTop: $('.homeGrid').offset().top
	    }, 700, function() {
  ...
  $('.closeLB').click(function() {
	...
	$('a.playVideo').click(function(e) {
		...
		videoSizeWidth(sizeVideo, (bWidth * .7));
		window.location.hash = slug;
		$('body').addClass('videoOpen');
		
		$('html, body').animate({
			scrollTop: $('.cell a[data-slug="'+slug+'"]').offset().top
		}, 500);
		
	});
  
	// VIDEO PAGE HASH VALUE LOAD
	...
	*/
    ...
    $('.intro').height(bHeight);
		...
		/* SKROLLR */
		if (bWidth > 768 && $('html').hasClass('no-touch')) {
			if ($('body').hasClass('home') || $('body').hasClass('skrollr')) {
				var s = skrollr.init();
				s.refresh();
			}
		}
	...
	*/
		...
		var introW = $('.vertscroll .thetext').width(),
			introTW = $('.vertscroll .thetext h1.title').outerWidth(true),
			introPW = (introW - introTW) - 10;
			
		$('.vertscroll .thetext p').width(introPW);
		
		$('.mobile iframe').each(function() {
			
			var video = $(this),
				ow = video.attr('width'),
				oh = video.attr('height'),
				r = ow / oh,
				nw = $(window).width(),
				nh = nw / r;
				
			video.width(nw).height(nh);
